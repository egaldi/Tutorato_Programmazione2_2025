...
/*Il Pig Latin è una lingua che consente una comunicazione cifrata:
Se una parola inzia per una o più consonti, il gruppo consonantico viene spostato alla fine della parola ed aggiunto il suffisso
‘ay’ (esempio: casa -> asacay ; gruppo -> uppogray)
Se una parola inizia per vocale, si aggiunge semplicemente il suffisso ‘ay’ alla fine della parola (esempio: oca -> ocaay:
ombrello -> ombrelloay)
Scrivere un programma che data in input una parola se produca il suo cifrato secondo le regole del Pig Latin*/
